<%-- 
    Document   : GestionCandidatoActualizar
    Created on : 02-sep-2019, 23:18:51
    Author     : migol
--%>
                ...
                <div id="content">
                    ...
                    <div class="container-fluid" style="width: 50%;">
                        <form method="post" action="Candidato">
                            <!-- Page Heading -->
                            <h1 class="h3 mb-4 text-gray-800">Actualizar candidato</h1>

                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Actualización del candidato</h6>
                                </div>
                                <div class="card-body">
                                    <%

                                        int id_candidato = Integer.parseInt(request.getParameter("id"));

                                        CandidatoVO candidatoVO = new CandidatoVO();
                                        CandidatoDAO candidatoDAO = new CandidatoDAO();

                                        ArrayList<CandidatoVO> listaCandidatosPropuestas = candidatoDAO.listarCandidatosActualizar(id_candidato);

                                        for (int i = 0; i < listaCandidatosPropuestas.size(); i++) {
                                            candidatoVO = listaCandidatosPropuestas.get(i);

                                    %>
                                    <%--<label for=""># [ID] Candidato</label>--%><input type="hidden" name="textid_candidato" class="form-control" value="<%=id_candidato%>" readonly>
                                    <label for="">Nombres</label><input type="text" name="textnombres" value="<%=candidatoVO.getNombres_candidato()%>" class="form-control" required=»required» pattern="[A-Za-z]+">
                                    <label for="">Primer apellido</label><input type="text" name="textprimerap" value="<%=candidatoVO.getPrimerapellido_candidato()%>" class="form-control" required=»required» pattern="[A-Za-z]+">
                                    <label for="">Segundo apellido</label><input type="text" name="textsegundoap" value="<%=candidatoVO.getSegundoapellido_candidato()%>" class="form-control" required=»required» pattern="[A-Za-z]+">
                                    <label for="">Tipo identificación</label><select id="txttipodoc" value="<%=candidatoVO.getTipoid_candidato()%>" name="texttipoid"   class="form-control" required=»required»>
                                        <option value = "<%=candidatoVO.getTipoid_candidato()%>"><%=candidatoVO.getTipoid_candidato()%></option>
                                    ...
                                    <label for="">Documento</label><input type="number" value="<%=candidatoVO.getDocumento_candidato()%>" name="textdocumento_candidato" class="form-control" required pattern="[0-9]+" >

                                    <label for="">Género</label><select id="selectgenero" name="textgenero"  value="<%=candidatoVO.getGenero_candidato()%>" class="form-control" required>
                                        <option value="<%=candidatoVO.getGenero_candidato()%>"><%=candidatoVO.getGenero_candidato()%></option>
                                        <option value="null">Elija una opcion...</option>
                                        <option value="Masculino">Masculino</option>
                                        <option value="Femenino">Femenino</option>
                                    </select>
                                    <label for="">Estado de candidato</label><select id="txtestadoca" name="textestado" value="<%=candidatoVO.getEstado_candidato()%>" class="form-control" required>
                                        <option value="<%=candidatoVO.getEstado_candidato()%>"><%=candidatoVO.getEstado_candidato()%></option>
                                        <option value="null">Elija una opcion...</option>
                                        <option value="Pendiente">Pendiente</option>
                                        <option value="Aprobado">Aprobado</option>
                                        <option value="No apto">No apto</option>    
                                    </select>  
                                    <label for="">Correo electrónico</label><input type="email" name="textcorreo" value="<%=candidatoVO.getCorreo_candidato()%>" class="form-control" pattern="[a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*@[a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{1,5}" required >
                                    <label for="">Jornada</label><select name="textjornada" value="<%=candidatoVO.getJornada()%>" class="form-control" required>
                                        <option value="<%=candidatoVO.getId_jornada()%>"><%=candidatoVO.getJornada()%></option>
                                        <option value="null">Elija una opcion...</option>
                                    ...
                                    <%}%>
                                    <div class="mb-2"><br>
                                        <button class="btn btn-warning">Guardar Configuración</button><br>
                                        <input type="hidden" name="opcion" value="2"> <br>

                                        <a href="GestionCandidato.jsp" class="btn btn-warning">Cancelar</a>
                                    </div>
                                    </form>
                                    <form method="post" action="Imagen" enctype="multipart/form-data">

                                        <input type="hidden" name="textid_candidato" value="<%= id_candidato%>">
                                        <label>Subir Imagen</label>
                                        <input type="file" class="form-control" name="filefoto"><br>

                                        <input type="submit" name="opcion" value="Subir" class="btn btn-warning">
